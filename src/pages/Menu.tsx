...
const MenuPage = () => {
  ...
  // Mock data for initial view if API fails
  const mockItems: FoodItemType[] = [
    { id: '1', name: 'Truffle Risotto', description: 'Arborio rice with black truffle shavings, 24-month aged parmesan, and micro-herbs.', price: 42, category: 'Main Course', image: 'https://images.unsplash.com/photo-1633964913295-ceb4c82487b9?auto=format&fit=crop&q=80&w=800', isVeg: true, rating: 4.8 },
    { id: '2', name: 'Gold Leaf Wagyu', description: 'Grade A5 Miyazakigyu beef topped with 24k edible gold leaf and sea salt.', price: 185, category: 'Main Course', image: 'https://images.unsplash.com/photo-1615937657715-bc7b4b7962c1?auto=format&fit=crop&q=80&w=800', isVeg: false, rating: 5.0 },
    { id: '3', name: 'Caviar Scallops', description: 'Hokkaido scallops seared in cultured butter, topped with Beluga caviar.', price: 65, category: 'Starters', image: 'https://images.unsplash.com/photo-1599458252204-763ff81b19b9?auto=format&fit=crop&q=80&w=800', isVeg: false, rating: 4.9 },
    { id: '4', name: 'Saffron Lobster', description: 'Butter-poached Brittany lobster in a velvety saffron-infused bisque.', price: 95, category: 'Main Course', image: 'https://images.unsplash.com/photo-1553163147-622ab57be1c7?auto=format&fit=crop&q=80&w=800', isVeg: false, rating: 4.7 },
    ...
    // New Desserts
    { id: '32', name: 'Golden Leaf Soufflé', description: 'Grand Marnier soufflé with edible 24k gold leaf and orange zest.', price: 34, category: 'Desserts', image: 'https://images.unsplash.com/photo-1579306194872-64d3b7bac4c2?auto=format&fit=crop&q=80&w=800', isVeg: true, rating: 4.9 },
    { id: '33', name: 'Exotic Fruit Carpaccio', description: 'Thinly sliced seasonal tropical fruits with a mint-lime reduction.', price: 28, category: 'Desserts', image: 'https://images.unsplash.com/photo-1546173159-319524d246b6?auto=format&fit=crop&q=80&w=800', isVeg: true, rating: 4.7 },
  ...
  useEffect(() => {
    // Forcing use of local 41-item list to ensure full menu visibility in demo mode
    setItems(mockItems);
    setLoading(false);
  }, []);
    ...
    <div className="pt-32 pb-24 px-6 min-h-screen bg-[#050505]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-white/40 tracking-[0.2em] uppercase text-xs">Exquisite flavors crafted for the elite</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-6 mb-12 items-center justify-between">
          <div className="flex bg-white/5 p-1 rounded-full border border-white/10 overflow-x-auto max-w-full">
            {categories.map(cat => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-xs uppercase tracking-widest transition-all whitespace-nowrap ${filter === cat ? 'bg-[#D4AF37] text-black font-bold' : 'text-white/60 hover:text-white'}`}
              >
                {cat}
              </button>
            ))}
          ...
          <div className="relative w-full md:w-64">
            ...
            <input 
              ...
              className="w-full bg-white/5 border border-white/10 rounded-full py-3 pl-12 pr-4 text-sm focus:border-[#D4AF37] outline-none transition-all text-white"
            />
          </div>
        </div>
          ...
          <AnimatePresence mode="popLayout">
            {filteredItems.map((item, i) => (
              <motion.div
                ...
                className="group bg-white/5 border border-white/10 rounded-3xl overflow-hidden backdrop-blur-sm hover:border-[#D4AF37]/50 transition-colors"
              >
                <div className="aspect-square relative overflow-hidden">
                  ...
                  {item.isVeg && (
                     <div className="absolute top-4 left-4 w-5 h-5 border-2 border-green-500 flex items-center justify-center rounded-sm bg-white/10">
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-[#D4AF37] font-mono text-lg">${item.price}</span>
                  </div>
                  <p className="text-white/40 text-sm mb-6 line-clamp-2">{item.description}</p>
                  <button 
                    onClick={() => handleAddToCart(item)}
                    className="w-full py-3 bg-white/10 hover:bg-[#D4AF37] hover:text-black transition-all rounded-xl flex items-center justify-center space-x-2 text-xs uppercase font-bold tracking-widest text-[#D4AF37] group-hover:bg-[#D4AF37] group-hover:text-black"
                  >
                    <ShoppingBag size={14} />
        ...
        {filteredItems.length === 0 && (
          <div className="text-center py-24">
            <p className="text-white/20 text-lg italic font-serif">No masterpieces found matching your search.</p>
          </div>
        )}
      </div>
    </div>
